...
void InsereNaPagina(Apontador Ap, Indice Ind, Apontador ApDir, int id){
	...
	while(NaoAchouPosicao){
		x = strcmpi(Ind.titulo, Ap->r[k-1].titulo);
		if(((id==1) && (Ind.codigo>=Ap->r[k-1].codigo)) || ((id==2) && (x>=0))){
...
void Ins(Indice Ind, Apontador Ap, short *Cresceu, Indice *IndRetorno, Apontador *ApRetorno, int id, short *inseriu){
	...
	while((id==1) && (i<Ap->n) && (Ind.codigo>Ap->r[i-1].codigo)) i++;
	while((id==2) && (i<Ap->n) && (strcmpi(Ind.titulo, Ap->r[i-1].titulo)>0)) i++;
	
	if(PesquisaArvore(Ind, Ap, 1) || PesquisaArvore(Ind, Ap, 2)){
		printf("Erro: Exemplar ja esta cadastrado.\n");    //fazer alteracao aqui
		system("pause");
		*inseriu = *Cresceu = FALSE;
		return;
	}
	
	if(((id==1) && (Ind.codigo<Ap->r[i-1].codigo)) || ((id==2) && (strcmpi(Ind.titulo, Ap->r[i-1].titulo)<0))) i--;
	
	Ins(Ind, Ap->p[i], Cresceu, IndRetorno, ApRetorno, id, inseriu);
...
void Ret(Indice Ind, Apontador *Ap, int *Diminuiu, int id, int *excluiu){
	...
	Pag = *Ap;
  	...
  	while(id==1&& i<Pag->n && Ind.codigo>Pag->r[i-1].codigo) i++;
  	while(id==2&& i<Pag->n && strcmpi(Ind.titulo, Pag->r[i-1].titulo)>0) i++;	  
	
	if((id==1 && Ind.codigo == Pag->r[i-1].codigo) || (id==2 && strcmpi(Ind.titulo, Pag->r[i-1].titulo)==0)){
		if(Pag->p[i-1] == NULL){
      		Pag->n--;
      		*Diminuiu = Pag->n < M;
      		for(j=i; j<=Pag->n; j++){
	...
	if((id==1&&Ind.codigo > Pag->r[i - 1].codigo)|| (id==2 && strcmpi(Ind.titulo, Pag->r[i-1].titulo)>0)) i++;
